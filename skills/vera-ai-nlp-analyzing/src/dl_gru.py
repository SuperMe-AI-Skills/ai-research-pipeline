...
def build_vocab_gru(
    ...
    """
    Build a word->index vocab from a list of cleaned texts.
    Reserve:
      0: PAD, 1: UNK
    """
    counter = Counter()
...
def texts_to_padded_indices(
    ...
    """
    Convert texts into a padded 2D numpy array of indices [n_samples, max_len].
    """
    n = len(texts)
    arr = np.zeros((n, max_len), dtype=np.int64)  # PAD = 0
...
class GRUDataset(Dataset):
    def __init__(
        ...
        """
        input_ids : [N, max_len] int64
        labels : [N]
        extra_features : [N, F] or None
        """
...
class GRUSentimentModel(nn.Module):
    def __init__(
        ...
        fc_input_dim = gru_output_dim + extra_feature_dim
    ...
    def forward(
        ...
        _, h_n = self.gru(emb)  # h_n: [num_layers*num_directions, B, hidden_dim]
...
def train_gru_model(
    ...
    """
    Train a GRU model on text (optionally + extra features).

    Early stopping:
      - Track best validation F1
      - If `patience` is not None, stop after `patience` epochs
        without improvement in val F1.

    Returns
    -------
    model : best GRUSentimentModel (on val F1)
    metrics : dict with 'best_val_f1', 'best_epoch'
    vocab : word_to_idx mapping
    history : dict of lists (train_loss, val_loss, train_f1, val_f1)
    """
    ...
    # Dataset & dataloaders
    train_dataset = GRUDataset(X_train_ids, y_train, extra_features=train_extra)
    val_dataset   = GRUDataset(X_val_ids,   y_val,   extra_features=val_extra)
...
def predict_gru(
    ...
    dataset = GRUDataset(X_ids, dummy_labels, extra_features=extra_features)
...
def random_search_gru(
    ...
    """
    Random search around train_gru_model.

    Returns
    -------
    best_model, best_vocab, best_params, best_metrics, best_history
    """
    import itertools
    ...
    best_f1 = -1.0
    best_model: Optional[GRUSentimentModel] = None
    best_vocab: Optional[Dict[str, int]] = None
    best_history: Optional[Dict[str, List[float]]] = None
    best_params: Optional[Dict[str, Any]] = None
    best_metrics: Optional[Dict[str, float]] = None

    print(f"[GRU] Random search over {len(sampled_combos)} combinations")

    for i, combo in enumerate(sampled_combos, 1):
        params = dict(zip(keys, combo))
        print(f"\n[GRU] Trial {i}/{len(sampled_combos)} params={params}")
    ...
    if best_model is None:
        raise RuntimeError("random_search_gru did not find any successful configuration.")

    print(f"\n[GRU] ✅ Best val F1 = {best_f1:.4f} with params: {best_params}")
