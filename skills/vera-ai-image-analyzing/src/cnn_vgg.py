...
def build_vgg16(
    ...
    """Load VGG-16 and replace the final classifier layer.

    The VGG-16 ``classifier`` is a :class:`nn.Sequential` with seven
    layers.  Only the last :class:`nn.Linear` (index 6) is replaced so
    that the two hidden FC layers (4096 -> 4096) are preserved.
    ...
    freeze_backbone : bool
        If ``True``, freeze all feature-extraction layers and the first
        six classifier layers.
    ...
    if pretrained:
        try:
            weights = getattr(models.VGG16_Weights, pretrained_weights)
        except AttributeError as exc:
            raise ValueError(
                f"Unknown VGG16 weights variant: {pretrained_weights!r}."
            ) from exc
    else:
        weights = None
    ...
    if freeze_backbone:
        ...
        # Freeze all classifier layers except the last
...
def predict_vgg(
    ...
    """Run inference with a trained VGG-16.
