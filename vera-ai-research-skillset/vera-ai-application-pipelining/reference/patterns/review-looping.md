...
- **MAX_ROUNDS = 4**
- **REVIEWER_MODEL = gpt-5.4** when the configured reviewer bridge supports model selection *(external mode only)*
...
Check the current runtime tool surface for an external reviewer bridge:
...
`AUTO_REVIEW.md`. The pipeline must NEVER abort because a reviewer bridge is
...
**Round 2+**: Delta through the same reviewer thread when the bridge supports
...
- Save reviewer thread ID for multi-round continuity when the external bridge supports it
