...
```mermaid
graph TD
    T1[T1_baseline<br/>data diagnostics + baseline model]
    T2[T2_ml / T2_transfer<br/>ML battery or transfer learning]
    T3[T3_deep / T3_advanced<br/>deep learning or advanced architectures]
    T4[T4_ensemble<br/>stacking / voting ensemble]
    T5[T5_subgroup<br/>per-subgroup evaluation]
    CONV[Convergence<br/>step 07 comparison]
...
**Parallelism rules:**
- T1, T2, T3 run in parallel at Stage 4 start (no dependencies).
- T4 waits for T2 AND T3 (ensemble needs both to complete).
- T5 waits for T1 (needs baseline predictions to compute subgroup disparity).
- Convergence (step 07) waits for all five tracks.
- Assembly (step 08) always last.

If a track fails, downstream tracks that depend on it are marked `stranded` in `PIPELINE_STATE.json`; tracks with no dependency on the failed track continue unaffected. The manuscript reports the gap explicitly rather than silently skipping.
