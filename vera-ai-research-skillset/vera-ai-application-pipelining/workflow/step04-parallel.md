...
**Important**: Workflow steps 01-03 live in the reviewing skill and steps 04-08
live in the generating skill. They are two separate skills for the same modality.
T1 (data diagnostics + baseline) reads from the reviewing skill; T2-T5 read from the
generating skill. See `reference/specs/skill-routing-table.md`.
...
2. **Relative to this orchestrator skill** — `REPO_ROOT` is the directory three
   ...
   path; strip the trailing `vera-ai-application-pipelining/workflow/step04-parallel.md`.)
3. **Sibling discovery** -- search upward from `$PWD` for the first ancestor
   directory that contains BOTH `vera-ai-{modality}-reviewing/` AND
   `vera-ai-{modality}-generating/`. That ancestor is `REPO_ROOT`.

Verify by checking that both resolved skill directories contain `SKILL.md` before
proceeding. If none of the methods resolves, abort with a clear error and ask
the user to set `AIRESEARCH_ROOT`.

```
REPO_ROOT      = <discovered as above>
TESTING_PATH   = {REPO_ROOT}/{testing_skill_path}            # e.g. vera-ai-nlp-reviewing
ANALYZING_PATH = {REPO_ROOT}/{analyzing_skill_path}             # e.g. vera-ai-nlp-generating

Read both skills:
  {TESTING_PATH}/SKILL.md
  {TESTING_PATH}/workflow/step01-collect-inputs.md
  {TESTING_PATH}/workflow/step02-check-distribution.md
  {TESTING_PATH}/workflow/step03-run-primary-test.md

  {ANALYZING_PATH}/SKILL.md
  {ANALYZING_PATH}/workflow/step04-run-additional-models.md
  {ANALYZING_PATH}/workflow/step05-analyze-subgroups.md
  {ANALYZING_PATH}/workflow/step06-fit-advanced-models.md
  {ANALYZING_PATH}/workflow/step07-compare-models.md
  {ANALYZING_PATH}/workflow/step08-generate-manuscript.md
  {ANALYZING_PATH}/reference/
  {ANALYZING_PATH}/scripts/python/       # reusable Python engine modules
```
...
vary by modality (see reference/specs/method-tracks.md).
...
3. **`spawn_agent` + `send_input` + `wait_agent`** (lifecycle-based
   runtimes, e.g. Codex): use only when the runtime policy and user
   authorization permit subagents. `dispatch_track(prompt)` →
   `id = spawn_agent(); send_input(id, prompt)`. Collect results by calling
   `wait_agent(id)` for each spawned id in section 4.3.
...
Follow reference/patterns/literature-reviewing.md to search across arXiv, Google Scholar, Semantic Scholar, and conference proceedings.
