...
if an external reviewer bridge is available:
    REVIEW_MODE = "external"   # use bridge, follow sections 7.1-7.7
else:
...
### 7.8 Self-Review Mode (No External Reviewer Bridge)

When `REVIEW_MODE = "self"` (no external reviewer bridge), the main agent acts
as its own reviewer. The procedure mirrors sections 7.1-7.7 but skips external
reviewer calls:
...
6. **Document the round** in `AUTO_REVIEW.md` per section 7.5, with
   `Reviewer: self (no external reviewer bridge)` instead of the external
   reviewer name. Save the full self-review verbatim under the "Reviewer Raw
   Response" block.
...
The skill should NEVER abort because an external reviewer bridge is missing —
self-review is the documented fallback and produces the same artifact set.
...
|----|------------|---------------|------------------|
| 7a | Review mode selected | REVIEW_MODE recorded in PIPELINE_STATE.json (`external` or `self`) | Auto-detect per section 7.0; default to `self` if no external reviewer bridge is available |
| 7a' | Self-review fallback wired | If REVIEW_MODE=self, follow section 7.8 instead of 7.1-7.6 | Never abort the pipeline because a bridge is missing |
...
- `AUTO_REVIEW.md` -- Full review history (project root)
