...
Implements:
  - train_xgboost: manual grid search with itertools.product
  - predict_xgboost: returns (y_pred, y_proba)

Supports both classification (XGBClassifier) and regression (XGBRegressor).
"""
...
def train_xgboost(
    ...
    """
    XGBoost training with manual grid search.
    ...
    if verbose:
        print(f"\n[XGBoost] Starting hyperparameter search over {len(all_param_combos)} combinations...")

    start_time = time.time()

    for i, params in enumerate(all_param_combos, start=1):
        if verbose:
            print(f"\n[XGBoost] Combination {i}/{len(all_param_combos)}: {params}")
        ...
        try:
            ...
            if verbose:
                print(f"[XGBoost] Failed with parameters {params} due to: {e}")
            continue

        if verbose:
            print(f"[XGBoost] Validation {metric_name}: {score:.4f}")
    ...
    if best_model is None:
        raise RuntimeError("No valid XGBoost model was found during tuning.")

    if verbose:
        print("\n[XGBoost] Hyperparameter search completed.")
        print(f"[XGBoost] Best params: {best_params}")
        print(f"[XGBoost] Best validation score: {best_score:.4f}")
        print(f"[XGBoost] Total tuning time: {total_time:.2f} seconds")
