# Review Looping: External or Self Review

Iterative external review and revision cycle for AI/ML research.
Works in two modes — **external** (configured reviewer bridge) and **self** (main agent
...
- **REVIEWER_MODEL = gpt-5.4** -- External reviewer through the configured reviewer bridge  *(external mode only)*
...
Check the current runtime tool surface for external reviewer bridge availability:
...
`AUTO_REVIEW.md`. The pipeline must NEVER abort because an external reviewer
bridge is missing — self-review is the documented fallback and produces the same
output artifact set.
...
**Round 2+**: Delta submission through the same reviewer thread when the bridge
supports threaded replies. With Codex MCP, use `mcp__codex__codex-reply`.
```
Since last review:
...
- If no external reviewer bridge is available at the Mode Detection step, this sub-skill
...
When no external reviewer bridge is in the tool surface, the main agent acts as its
...
```markdown
## Round N ({timestamp})
### Reviewer: self (no external reviewer bridge)
### Score: X/10
### Verdict: {verdict}
