...
- `external` -> the configured reviewer bridge may assist with brainstorming, novelty cross-checking, and idea review
- `local` -> the main agent performs those tasks directly from search evidence and local reasoning

Step 02 MUST NOT abort just because no external reviewer bridge is available.
...
- Cross-verify with the configured reviewer bridge if available; otherwise perform a structured self-cross-check against the closest papers found
...
| 2c | Novelty checked | Top ideas have novelty scores, with or without an external reviewer bridge | Proceed with caveat |
